...
class SadAdapter(_songDetails: ArrayList<Songs>, _context: Context) : RecyclerView.Adapter<SadAdapter.MyViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SadAdapter.MyViewHolder {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.row_custom_mainscreen_adapter, parent, false)
        return MyViewHolder(itemView)
    }

    override fun onBindViewHolder(holder: SadAdapter.MyViewHolder, position: Int) {
            ...
            popup.setOnMenuItemClickListener { item ->
                when (item.itemId) {
                    R.id.it_delete -> {
                        ...
                        //add particular item to Sad list

                        if (SadFragment.Statified.sadContent?.checkifIdExistsSad(songId.toInt()) as Boolean) {
                            SadFragment.Statified.sadContent?.deleteSad(songId.toInt())
